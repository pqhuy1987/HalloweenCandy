...
bool GameScene::init()
{
    ...
    score_bar->setPosition(score_bar->getContentSize().width/2 + origin.x, visibleSize.height * 0.8 + origin.y);
    this->addChild(score_bar,50);
    
    __String *tempScore = cocos2d::__String::createWithFormat("%i", m_score);
    scoreLabel = Label::createWithTTF(tempScore->getCString(), "Lucky7.otf", visibleSize.height * SCORE_FONT*0.3);
    scoreLabel->setColor(Color3B::WHITE);
    scoreLabel->setPosition(score_bar->getContentSize().width/2+ score_bar->getContentSize().width*0.1+ origin.x, visibleSize.height * 0.795 + origin.y);
...
void GameScene::scrollBk(float delta)
{
    ...
    this->schedule(schedule_selector(GameScene::spawnCloud), CLOUD_SPAWN_FREQ*visibleSize.height,kRepeatForever,60);
